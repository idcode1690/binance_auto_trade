(async()=>{
  try {
    ...
    // inspect SVG polylines (EMA lines) for dash attributes or counts
    try{
      const svgInfo = await page.evaluate(()=>{
        ...
        return {
          ...
          polylines: polylines.map(p => ({ points: p.getAttribute('points') ? p.getAttribute('points').split(' ').length : 0, stroke: p.getAttribute('stroke'), strokeWidth: p.getAttribute('stroke-width') || p.getAttribute('strokeWidth') || null, strokeDasharray: p.getAttribute('stroke-dasharray') }))
        }
      })
      logs.svg = svgInfo
    } catch(e) { logs.errors.push({type:'evaluate-svg',message: e.message}) }
